            ...
            loading ? (
             <div className='flex items-center justify-center h-[80vh]'>
                ...
                <div className='flex flex-wrap gap-3.5'>
                      {
                        projects.map((project)=>(
                           <div onClick={()=> navigate(`/projects/${project.id}`)} key={project.id} className='relative  group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:drop-shadow-indigo-700/3 hover:border-indigo-800/80 transition-all duration-300'>
                              {/**  Desktop-like Mini Preview  */}
                              <div className='relative w-full h-40 bg-gray-900 overflow-hidden border border-b border-gray-800'> 
                                    {project.current_code ? (
                                      <iframe
                                       srcDoc={project.current_code } className='absolute top-0 left-0 w-[1200px] h-[800px] origin-top-left pointer-events-none' sandbox='allow-scripts allow-same-origin'
              ...
              <div className='flex flex-col items-center justify-center h-[80vh]'>
                  ...
                  <button onClick={() => navigate('/')} className='text-while px-5 mt-5 rounded-md bg-indigo-500 hover:bg-indigo active:scale-95 transition-all'>
                       Create New
                  </button>
              </div>
            )
